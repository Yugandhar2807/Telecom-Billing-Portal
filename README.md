...
3. Once loaded, run: `npm run install-deps && npm run dev`
...
2. Add MySQL addon: `heroku addons:create cleardb:ignite`
...
3. Use the included `build.sh` script

### Docker Deployment
```bash
# Build the image
docker build -t telecom-billing-portal .

# Run the container
docker run -p 8000:8000 -e DB_HOST=your-db-host -e DB_PASSWORD=your-password telecom-billing-portal
```

### Local Development
```bash
git clone https://github.com/Yugandhar2807/Telecom-Billing-Portal.git
cd Telecom-Billing-Portal
npm run install-deps
